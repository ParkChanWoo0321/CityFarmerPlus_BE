...
public record ParticipationFormRequest(
        @NotNull(message = "농업경영체 등록 여부를 입력해야 합니다.")
        Boolean agriculturalBusinessRegistered,

        @NotNull(message = "농업 작업 경험 횟수를 입력해야 합니다.")
        @Min(value = 0, message = "농업 작업 경험 횟수는 0 이상이어야 합니다.")
        @Max(value = 10000, message = "농업 작업 경험 횟수는 10000 이하여야 합니다.")
        Integer experienceCount,

        @Size(max = 1000, message = "경험 특이사항은 1000자 이하여야 합니다.")
        ...
        @Size(max = 1000, message = "희망 근무 특이사항은 1000자 이하여야 합니다.")
        String workPreferenceNotes,

        @Size(max = 1000, message = "신청 특이사항은 1000자 이하여야 합니다.")
        String applicationNote,

        @PositiveOrZero(message = "신청서 버전은 0 이상이어야 합니다.")
        Long expectedApplicationVersion,

        @PositiveOrZero(message = "프로필 버전은 0 이상이어야 합니다.")
        Long expectedProfileVersion,

        @PositiveOrZero(message = "희망 근무 조건 버전은 0 이상이어야 합니다.")
        Long expectedWorkPreferenceVersion
) {
