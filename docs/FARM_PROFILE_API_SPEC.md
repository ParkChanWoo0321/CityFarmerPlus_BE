...
- 농가 소유 심사 상태는 공고 작성·공개·도시농부 지원의 자격 조건이 아니다. 활성 `FARM` 계정이 프로필을 저장하면 상태가 `DRAFT`여도 공고 초안을 작성하고 심사를 요청할 수 있다. 지원자 의견과 출결·근무 완료 관리는 `APPROVED` 농가만 가능하다.
...
농가 심사 목록·상세·승인·반려와 담당자 증빙 다운로드 API가 `/api/admin/farm-profiles/**`에 통합돼 있다. 사용자 API는 같은 모델에 저장된 심사 결과를 조회하지만, `APPROVED` 여부를 공고 작성·공개·도시농부 지원 자격에는 사용하지 않는다. 지원자 의견과 출결·근무 완료 관리는 승인 상태를 계속 확인한다. 상세 계약은 `ADMIN_FARM_OWNERSHIP_API_SPEC.md`를 따른다.
