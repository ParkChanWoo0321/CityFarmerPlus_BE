...
- 프로필 소유자는 요청값이 아니라 JWT의 `sub`(회원 ID, `Authentication.getPrincipal()`)로 결정한다.
...
인증(`401`)·인가(`403`) 오류는 이 형식을 따르지 않는다. 자세한 내용은 5장을 참고한다.
...
| 상태 | 발생 조건 |
|---|---|
| `400` | 필수값 누락(공백 포함) 또는 `experienceCount` 음수, `introduction` 500자 초과 |
| `400` | 이미 프로필이 등록된 계정으로 재등록 시도 |
| `401` | JWT 누락, 만료 또는 위조 |
| `403` | `URBAN_FARMER`가 아닌 계정(`FARM`, `CENTER_ADMIN`)의 JWT로 접근 |
...
| 상태 | 발생 조건 |
|---|---|
| `400` | 필수값 누락 또는 형식·길이 위반 |
| `400` | 아직 프로필을 등록하지 않은 계정으로 수정 시도 |
| `401` | JWT 누락, 만료 또는 위조 |
| `403` | `URBAN_FARMER`가 아닌 계정의 JWT로 접근 |

## 6. 인증·인가 오류 응답 주의사항

이 프로젝트의 `SecurityConfig`는 `401`에 대해서만 커스텀 처리를 하며, 이마저도 `{ "message" }` 형식이 아니다.

- `401`: `authenticationEntryPoint`가 `response.sendError(401, "인증이 필요합니다.")`를 호출한다. 실제 응답 본문은 Spring Boot 기본 에러 핸들러(`BasicErrorController`)가 생성하며 `{ "message" }` 형식을 따르지 않는다.
- `403`: 별도의 `AccessDeniedHandler`를 등록하지 않았으므로 Spring Security 기본 처리(Spring Boot 기본 에러 핸들러 경유)를 따른다. 마찬가지로 `{ "message" }` 형식이 아니다.
